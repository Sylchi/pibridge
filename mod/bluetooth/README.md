...
|                                  | Linux              | macOS              | Windows            | Nordic Semi        | ESP32 (NINA-FW)    | CYW43439 (RP2040-W) | BCM43438 (Pi Zero W) |
| -------------------------------- | ------------------ | ------------------ | ------------------ | ------------------ | ------------------ | ------------------- | -------------------- |
...
| Connect to peripheral            | :heavy_check_mark: | :heavy_check_mark: | :heavy_check_mark: | :heavy_check_mark: | :heavy_check_mark: | :heavy_check_mark:  | :x:                 |
| Write peripheral characteristics | :heavy_check_mark: | :heavy_check_mark: | :heavy_check_mark: | :heavy_check_mark: | :heavy_check_mark: | :heavy_check_mark:  | :x:                 |
| Receive notifications            | :heavy_check_mark: | :heavy_check_mark: | :heavy_check_mark: | :heavy_check_mark: | :heavy_check_mark: | :heavy_check_mark:  | :x:                 |
...
| Send notifications               | :heavy_check_mark: | :x:                | :heavy_check_mark: | :heavy_check_mark: | :heavy_check_mark: | :heavy_check_mark:  | :x:                 |
...
Go Bluetooth has bare metal support for the Raspberry Pi Zero W, which uses
the BCM43438 (CYW43438) combo WiFi/BT chip. This is the same HCI-based
co-processor interface as the CYW43439 on the RP2040-W.

The BCM43438 requires firmware to be loaded into the chip's RAM via HCI
vendor commands before the BLE stack can be enabled. The firmware blob is
typically extracted from the Linux `brcmfmac43430-sdio.bin` or
`brcmfmac43436-sdio.bin` driver package.

### Usage

```go
// Power on the BT chip, load firmware, then enable the adapter
btPowerOn()
btLoadFirmware(uart)
adapter.Enable()
```
...
- **UART:** PL011 (UART0) on GPIO 32/33 at 115200 baud
- **Power:** GPIO 45 must be driven high to enable the BCM43438
- **Firmware loading:** HCI vendor commands via UART before standard HCI init

## BCM43438 Known Limitations

- Only **Peripheral (server)** mode is currently supported.
- Central/client operations (scan, connect) are not available.
- The firmware must be pre-loaded via HCI vendor commands; the driver does
  not include a built-in firmware loader for all board variants.
